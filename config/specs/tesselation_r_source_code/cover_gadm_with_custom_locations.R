...
capture.output(
  for (i in 1:nrow(cntries_df)) {
    ...
    ptm0 <- proc.time()
    invalid_list <- determine_invalid_custom_locations(geo_custom_list=res_list)
    ptm1 <- proc.time()
    ...
    expanded_coverage <- expand_coverage_for_sparse_locations(geo_shape_file = geo_adm2, 
                                                              idcol = paste0('GID_',gadm_level),
                                                              geo_custom_list = res_list, 
