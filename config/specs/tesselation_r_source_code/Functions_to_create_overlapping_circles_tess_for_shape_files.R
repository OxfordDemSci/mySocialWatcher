...
# Similar to show_custom_location_covers_on_map but doesnt aggregate the buffer together - for manual inspection
...
# Adapt get_psw for mySocialWatcher and datahub format

get_mysw_geo_queries <- function(geo_covers_df, cover_type, country_code='', geo_source='GADM', location_type='recent', expanded_type=expanded){
  ...
  for (geoid in levels(as.factor(geo_covers_df$geo_id))) {
    ...
    geo_query <- paste('{"name":"custom_locations", "values":[',custom_query,'],',
                       '"location_types": ["',location_type,'"], "pySocialWatcherReference": {"geo_id": "',
                       geoid,'" ,"geo_source": "',geo_source,'", "coverType": "',cover_type,
                       '" , "expanded": "',expanded_type,'"},',
                       '"country_code":"',country_code,'"}',sep="")
    all_geo_queries <- c(all_geo_queries, geo_query)
