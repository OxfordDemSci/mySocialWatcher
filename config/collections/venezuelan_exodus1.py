...
if __name__ == '__main__':
    ...
    for i in range(len(specs_template.get('geo_locations'))):
        ...
        if country == 'CO' and loc.get('name') == 'regions':
            specs['geo_locations'].append(loc)

    # write json to file
    file_out = os.path.join(specs_dir, '1_ven_exodus_regions_CO.json')
    ...
    # write json to file
    file_out = os.path.join(specs_dir, '2_ven_exodus_country_CO.json')
    with open(file_out, "w") as f:
        f.write(json.dumps(specs))
    ...
    for i in range(len(specs_template.get('geo_locations'))):
        loc = specs_template.get('geo_locations')[i]
        country = country_from_location(loc)
        if country == 'CO' and loc.get('name') == 'cities':
            specs['geo_locations'].append(loc)

    # write json to file
    file_out = os.path.join(specs_dir, '3_ven_exodus_cities_CO.json')
    with open(file_out, "w") as f:
        f.write(json.dumps(specs))
